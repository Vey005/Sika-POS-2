...
const sikapos = {
  ...
  /** OS / title-bar close was intercepted; show attendance prompt then call window.confirmClose(). */
  onCloseIntercepted: (callback: () => void) => {
  ...
  users: {
    ...
    login: (password: string) => ipcRenderer.invoke('users:login', password),

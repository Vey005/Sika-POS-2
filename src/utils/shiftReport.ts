/** Build shift report payloads for print / PDF (POS + portal). */
...
export function shiftDurationMinutes(clockIn: string, clockOut?: string | null): number {
...
export function formatShiftDuration(clockIn: string, clockOut?: string | null): string {
  const mins = shiftDurationMinutes(clockIn, clockOut);
  const h = Math.floor(mins / 60);
  const m = mins % 60;
  return `${h}h ${m}m`;
}

/** YYYY-MM-DD from clock-in (used for PDF filenames). */
...
/** Display date on report header (same style as daily EOD date line). */
...
/** Shift time range shown under staff name on EOD-style reports. */
...
export type ShiftReportReceiptConfig = {
  showLogo?: boolean;
  showCashier?: boolean;
  ...
  currency?: string;
  paperSize?: string;
  template?: 'standard' | 'compact' | 'elegant' | string;
};
...
export function buildShiftReportPayload(opts: {
  ...
  currency: string;
  receiptConfig?: ShiftReportReceiptConfig;
}) {
  const { log, summary, transactions, businessName, businessLogo, currency, receiptConfig } = opts;
  const normalized = normalizeShiftSummary(summary);
  const itemSummary = buildItemSummaryFromTransactions(transactions);
